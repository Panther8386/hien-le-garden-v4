...
- Migration file: `migrations/0004_bookings_and_rooms.sql`, applied locally via the existing `vitest.config.js`/`test/apply-migrations.js` machinery automatically — applying it to the **real remote D1** (`wrangler d1 migrations apply hien_le_garden_crm --remote`) must happen once, **before** this branch merges to `main` and the auto-deploy triggers, not after. The moment this branch's code deploys, the homepage's booking modal starts calling `/api/availability` and `/api/bookings`, which would throw `no such table` errors against an unmigrated database — breaking the public homepage's booking flow, not just admin pages. This matches `BACKEND.md`'s existing "Applying a new migration to production" guidance: apply the migration first, then merge/deploy.
