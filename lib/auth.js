...
export async function getSession(db, token) {
  const row = await db
    .prepare(
      `SELECT s.staff_id AS staffId, a.username, a.role, a.can_manage_room_layout AS canManageRoomLayout, a.can_add_finance_transaction AS canAddFinanceTransaction, a.can_delete_asset AS canDeleteAsset, a.can_delete_deposit AS canDeleteDeposit, a.totp_enabled AS totpEnabled FROM sessions s
  ...
  if (!row) return null;
  return { staffId: row.staffId, username: row.username, role: row.role, canManageRoomLayout: !!row.canManageRoomLayout, canAddFinanceTransaction: !!row.canAddFinanceTransaction, canDeleteAsset: !!row.canDeleteAsset, canDeleteDeposit: !!row.canDeleteDeposit, totpEnabled: !!row.totpEnabled };
}
